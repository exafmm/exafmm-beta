...
void relocate_data_radix6_hr(uint* pointIds, uint* index, uint long* zcodes, uint long* codes, int* str, int P, int M, int N, int sft){

#pragma ivdep
  for(int j=0; j<M; j++){
    if(P+j<N){
      uint ii = (zcodes[j]>>sft) & MASK64;
      int jj = str[ii];
      codes[jj] = zcodes[j];
      //pointIds[jj] = index[j];
      pointIds[jj] = P + j;
...
void bin_sort_serial_radix6_hr(uint long *zcodes, uint long* codes, uint *pointIds, uint* index, uint long* bins, int *level, int N, int sft, int tid, int lv, int stop){
  ...
  if(N<=th){
    ...
    if(lv<2){
      ...
      for(int i=0; i<MAXBINS64; i++){
        cilk_spawn bin_sort_serial_radix6_hr(&zcodes[acm_sizes[i]], &codes[acm_sizes[i]], &pointIds[acm_sizes[i]], &index[acm_sizes[i]], &bins[acm_sizes[i]], &level[acm_sizes[i]], BinSizes[i], sft-6, 64*tid + i, lv+1, stop);
      }
      cilk_sync;
    }
    else{
      for(int i=0; i<MAXBINS64; i++){
        bin_sort_serial_radix6_hr(&zcodes[acm_sizes[i]], &codes[acm_sizes[i]], &pointIds[acm_sizes[i]], &index[acm_sizes[i]], &bins[acm_sizes[i]], &level[acm_sizes[i]], BinSizes[i], sft-6, 64*tid + i, lv+1, stop);
      }
...
#pragma ivdep
      for(int j=0; j<M; j++){
        if(i*M+j<N){
          uint ii = (zcodes[i*M + j]>>sft) & MASK64;
          BinSizes[i*MAXBINS64 + ii]++;
        }
      }
    }
...
#pragma ivdep
    ...
    for(int i=0; i<NP; i++){
      //cilk_spawn relocate_data_radix6_hr(pointIds, &index[i*M], &zcodes[i*M], codes, &str[i*MAXBINS64], i*M, M, N, sft);
      cilk_spawn relocate_data_radix6_hr(permut, &index[i*M], &zcodes[i*M], codes, &str[i*MAXBINS64], i*M, M, N, sft);
    }
    cilk_sync;
    ...
    */
    ...
    for(int i=0; i<MAXBINS64; i++){
      cilk_spawn bin_sort_serial_radix6_hr(&zcodes[acm_sizes[i]], &codes[acm_sizes[i]], &pointIds[acm_sizes[i]], &index[acm_sizes[i]], &bins[acm_sizes[i]], &level[acm_sizes[i]], Sizes[i], sft-6, 64*tid + i, lv+1, stop);
    }
    cilk_sync;
...
void bin_sort_serial2(uint long *zcodes, uint long* codes, uint *pointIds, uint* index, uint long* bins, int *level, int N, int sft, int tid, int lv){

  int BinSizes[MAXBINS];
  int str[MAXBINS];
  uint acm_sizes[MAXBINS];
  uint* tmp_ptr; 
  uint long* tmp_code;
  ...
  if(N<=th){
	...
	cilk_spawn bin_sort_serial2(&zcodes[acm_sizes[i]], &codes[acm_sizes[i]], &pointIds[acm_sizes[i]], &index[acm_sizes[i]], &bins[acm_sizes[i]], &level[acm_sizes[i]], BinSizes[i], sft-3, 8*tid + i, lv+1);
      } 
      cilk_sync;
    }
    else{
      for(int i=0; i<MAXBINS; i++){
        bin_sort_serial2(&zcodes[acm_sizes[i]], &codes[acm_sizes[i]], &pointIds[acm_sizes[i]], &index[acm_sizes[i]], &bins[acm_sizes[i]], &level[acm_sizes[i]], BinSizes[i], sft-3, 8*tid + i, lv+1);
      }
...
void bin_sort3(uint long *zcodes, uint long* codes, uint *pointIds, uint* index, uint long* bins, int *level, int N, int sft, int tid, int lv){

  int BinSizes[NP3*MAXBINS];
  int str[NP3*MAXBINS];
  uint Sizes[MAXBINS];
  uint acm_sizes[MAXBINS+1];
  uint* tmp_ptr;
  uint long* tmp_code;
  //uint dd[MAXBINS];                                                                                                                                                                                        
  ...
  if(N<th){
    ...
    // Find which child each point belongs to                                                                                                                                                                
    cilk_for(int i=0; i<NP3; i++){
#pragma ivdep
      for(int j=0; j<M; j++){
        if(i*M+j<N){
          uint ii = (zcodes[i*M + j]>>sft) & 0x07;
          BinSizes[i*MAXBINS + ii]++;
        }
      }
    }
...
#pragma ivdep
      for(int j=1; j<NP3; j++){
        str[j*MAXBINS+i] = str[(j-1)*MAXBINS+i] + BinSizes[(j-1)*MAXBINS+i];
        Sizes[i] += BinSizes[(j-1)*MAXBINS+i];
      }
      dd = str[(NP3-1)*MAXBINS+i] + BinSizes[(NP3-1)*MAXBINS + i];
      Sizes[i] += BinSizes[(NP3-1)*MAXBINS + i];
    }

    for(int i=0; i<NP3; i++){
      cilk_spawn relocate_data(pointIds, &index[i*M], &zcodes[i*M], codes, &str[i*MAXBINS], i*M, M, N, sft);
    }
    cilk_sync;
    ...
    for(int i=0; i<MAXBINS; i++){
      cilk_spawn bin_sort_serial2(&zcodes[acm_sizes[i]], &codes[acm_sizes[i]], &pointIds[acm_sizes[i]], &index[acm_sizes[i]], &bins[acm_sizes[i]], &level[acm_sizes[i]], Sizes[i], sft-3, 8*tid + i, lv+1);
    }
    cilk_sync;
...
#pragma ivdep
    ...
    /*
    for(int i=0; i<NP; i++){
      cilk_spawn point_count(&zcodes[i*M], &BinSizes[i*MAXBINS], i*M, M, N, sft);
    }
    cilk_sync;
    */
    // scan prefix (must change this code)
...
#pragma ivdep
	...
	Sizes[i] += BinSizes[(j-1)*MAXBINS+i];
      ...
      dd = str[(NP-1)*MAXBINS+i] + BinSizes[(NP-1)*MAXBINS + i];
      Sizes[i] += BinSizes[(NP-1)*MAXBINS + i];
    }
    
    ///////////////////////
    /*
    for(int j=1; j<NP; j++){
      str[j*MAXBINS:MAXBINS] = str[(j-1)*MAXBINS:MAXBINS] + BinSizes[(j-1)*MAXBINS:MAXBINS];
    }
    
    acm_sizes[0] = 0;
    for(int i=1; i<MAXBINS; i++){

      uint tt = acm_sizes[i-1] + str[(NP-1)*MAXBINS + i-1] + BinSizes[(NP-1)*MAXBINS + i-1];
      acm_sizes[i] = tt;
      Sizes[i-1] = tt;
    }
    Sizes[MAXBINS-1] =  acm_sizes[MAXBINS-1] + str[(NP-1)*MAXBINS + MAXBINS-1] + BinSizes[(NP-1)*MAXBINS + MAXBINS-1];
    
    for(int j=0; j<NP; j++){
      str[j*MAXBINS:MAXBINS] += acm_sizes[0:MAXBINS]; 
    }
    */

    // Rearrange the points to the clusters
    /*
    cilk_for(int i=0; i<NP; i++){
#pragma ivdep
      for(int j=0; j<M; j++){
	if(i*M+j<N){
	  uint ii = (zcodes[i*M + j]>>sft) & 0x07;
	  pointIds[str[i*MAXBINS + ii]] = index[i*M + j];
	  codes[str[i*MAXBINS + ii]] = zcodes[i*M + j];
	  str[i*MAXBINS + ii]++;	
	}
      }
    }
    */
    ...
    for(int i=0; i<MAXBINS; i++){
      cilk_spawn bin_sort3(&zcodes[acm_sizes[i]], &codes[acm_sizes[i]], &pointIds[acm_sizes[i]], &index[acm_sizes[i]], &bins[acm_sizes[i]], &level[acm_sizes[i]], Sizes[i], sft-3, 8*tid + i, lv+1);
    } 
    cilk_sync;
