...
void test() {
  ...
  for (int i=0; i<S; i++) {
    ...
    assert(std::abs(b[i]-2)/2<EPS);
    assert(std::abs(c[i]-i)/i<EPS);
  }
  a = 2;
  b = c;
  c += 1;
  for (int i=0; i<S; i++) {
    ...
    assert(std::abs(b[i]-i)/i<EPS);
    assert(std::abs(c[i]-i-1)/(i+1)<EPS);
  }
  a -= 1;
  b *= 2;
  c /= 10;
  for (int i=0; i<S; i++) {
    assert(std::abs(a[i]-1)<EPS);
    assert(std::abs(b[i]-2*i)/(2*i)<EPS);
    assert(std::abs(c[i]-(i+1.)/10)*10/(i+1.)<EPS);
  }
  b &= c > (a / 2);
  a = b + c;
  c = a - b;
  for (int i=0; i<S; i++) {
    if (i < 5) {
      assert(std::abs(a[i]-(i+1.)/10)*10/(i+1.)<EPS);
      assert(std::abs(b[i])<EPS);
    } else {
      assert(std::abs(a[i]-(2.1*i+0.1))/(2.1*i+0.1)<EPS);
      assert(std::abs(b[i]-(2*i))/(2*i)<EPS);
    }
    assert(std::abs(c[i]-(i+1.)/10)*10/(i+1.)<EPS);
  }
  ...
  c = -b;
  for (int i=0; i<S; i++) {
    assert(std::abs(a[i]-(i+1.)*(i+1.)/100)*100/(i+1.)/(i+1.)<EPS);
    assert(std::abs(b[i]-(i+1.)/10)*10/(i+1.)<EPS);
    assert(std::abs(c[i]+(i+1.)/10)*10/(i+1.)<EPS);
  }
  a = rsqrt(a);
  b = min(a,b);
  c = max(a,c);
  for (int i=0; i<S; i++) {
    assert(std::abs(a[i]-10/(i+1.))/(i+1.)/10<EPS);
    assert(std::abs(b[i]-std::min((i+1.)/10,double(a[i])))/std::min((i+1.)/10,double(a[i]))<EPS);
    assert(std::abs(c[i]-10/(i+1.))/(i+1.)/10<EPS);
  }
  a = a[0]/5;
  b = sum(a);
  c = norm(a);
