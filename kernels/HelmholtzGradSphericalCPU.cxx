    ...
    void P2M(C_iter C) {
      ...
      for (B_iter B=C->BODY; B!=C->BODY+C->NBODY; B++) {
        ...
        for (int n=1; n<P; n++) {
          ...
          Mnm[nm+2*P*P] -= jnuse * Ynmd[nms] * stheta;
          for (int m=1; m<=n; m++) {
            nms = n * (n + 1) / 2 + m;
            int npm = n * n + n + m;
            int nmm = n * n + n - m;
            Mnm[npm] += Ynm[nms] * jn[n] * stheta * conj(ephi[m]);
            Mnm[nmm] += Ynm[nms] * jn[n] * stheta * ephi[m];
            Mnm[npm+P*P] += jnd[n] * Ynm[nms] * stheta * conj(ephi[m]);
            Mnm[nmm+P*P] += jnd[n] * Ynm[nms] * stheta * ephi[m];
            Mnm[npm+2*P*P] -= jnuse * Ynmd[nms] * conj(ephi[m]);
            Mnm[nmm+2*P*P] -= jnuse * Ynmd[nms] * ephi[m];
            Mnm[npm+3*P*P] -= jnuse * Ynm[nms] * real_t(m) * I * conj(ephi[m]);
            Mnm[nmm+3*P*P] += jnuse * Ynm[nms] * real_t(m) * I * ephi[m];
          }
        }
      }
      C->M += Mnm * I * wavek;
    ...
    void M2L(C_iter Ci, C_iter Cj, bool mutual) {
      ...
      for (int i=0; i<4; i++){
        ...
        for (int n=0; n<Popt; n++) {
          for (int m=-n; m<=n; m++) {
            int nm = n * n + n + m;
            Ci->L[nm+i*P*P] += ephi[P-m] * Lrot[nm];
          }
        }
      }
    }
    ...
    void L2P(C_iter C) {
      ...
      for (B_iter B=C->BODY; B!=C->BODY+C->NBODY; B++) {
        ...
        for (int n=0; n<P; n++) {
          ...
          int nms = n * (n + 1) / 2;
          TRG[0] += Lj[nm] * jn[n] * Ynm[nms];
          ur += Lj[nm+P*P] * jn[n] * Ynm[nms];
          utheta += Lj[nm+2*P*P] * jn[n] * Ynm[nms];
          uphi += Lj[nm+3*P*P] * jn[n] * Ynm[nms];                
          for (int m=1; m<=n; m++) {
            int npm = n * n + n + m;
            int nmm = n * n + n - m;
            int nms = n * (n + 1) / 2 + m;
            ...
            complex_t ztmp3 = Lj[nmm] * conj(ephi[m]);
            complex_t ztmpsum = ztmp2 + ztmp3;
            TRG[0] += jn[n] * Ynm[nms] * stheta * (Lj[npm] * ephi[m] + Lj[nmm] * conj(ephi[m]));
            ur += jn[n] * Ynm[nms] * stheta * (Lj[npm+P*P] * ephi[m] + Lj[nmm+P*P] * conj(ephi[m]));
            utheta += jn[n] * Ynm[nms] * stheta * (Lj[npm+2*P*P] * ephi[m] + Lj[nmm+2*P*P] * conj(ephi[m]));
            uphi += jn[n] * Ynm[nms] * stheta * (Lj[npm+3*P*P] * ephi[m] + Lj[nmm+3*P*P] * conj(ephi[m]));
          }
        }
        std::cout << TRG[0]<<" "<<ur << " " << utheta << " " << uphi << std::endl;
#endif
        ...
        std::cout<<TRG[0]<<" "<<ux<<" "<<uy<<" "<<uz<<std::endl;
