...
void upward_pass(float *X2, int **multipoles, int **node_codes2, int** c_count2, int** node_pointers2,
  ...
  if(c_size==0){ // P2M
    multipoles[level][node_id] = leaf_populations2[node_pointers2[level][node_id]];
    ...
    for(int i=c_begin; i<c_end; i++){
      upward_pass(X2, multipoles, node_codes2, c_count2, node_pointers2, leaf_populations2,
		  Xmin, Xmax, i, level+1);
      multipoles[level][node_id] += multipoles[level+1][i];
    }
  }
}

void downward_pass(float *X, int **locals, int **node_codes, int** c_count, int** node_pointers,
  ...
  if(c_size==0){ // L2P
    ...
    for(int i=c_begin; i<c_end; i++){
      locals[level+1][i] += locals[level][node_id];
