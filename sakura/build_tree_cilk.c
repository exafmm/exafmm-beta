...
#else
	node_codes[j][cursor[j]] = leaf_morton_codes[i] >> (3*(maxlev - j -1));
#endif
...
#else
      node_codes[L-1][cursor[L-1]] = leaf_morton_codes[i] >> (3*(maxlev - L));
#endif
      cursor[L-1]++;
      child_counter[L-1]++;
    }
...
void parent_children_connection_wrapper(int (**restrict node_pointers), 
  ...
  uint32_t *masks = (uint32_t *)malloc(L*sizeof(uint32_t));  
  int M = (int)ceil((float)N / (float)NP3); // Number of elements that belong to each thread
  //int M = N / NP3;
  cilk_for(int i=0; i<L; i++){
    masks[i] = 1 << i;
  }
...
/* A simple function to collect the new level codes and count the children of each node */
