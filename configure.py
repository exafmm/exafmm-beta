...
class ConfigurationManager(object):
  def __init__(self, project = 'ExaFMM'):
  ...
  def checkPythonVersion(self):
    if not hasattr(sys, 'version_info') or not sys.version_info[0] == 2 or not sys.version_info[1] >= 3:
      ...
      print '*                                                                             *'
      print '*           You CANNOT configure ExaFMM without Python                        *'
  ...
  def validateOptions(self, opts):
    '''This should be overridden for ExaFMM check_for_option_mistakes()+check_exafmm_arch()'''
    return
  ...
  def configure(self, configure_options):
    ...
    try:
      ...
      if e.code is None or e.code == 0:
        return
      msg ='*******************************************************************************\n'\
          +'         CONFIGURATION FAILURE  (Please send configure.log to exafmm-maint@kaust.sa)\n' \
          +'*******************************************************************************\n'
      se  = str(e)
    except Exception, e:
      msg ='*******************************************************************************\n'\
          +'        CONFIGURATION CRASH  (Please send configure.log to exafmm-maint@kaust.sa)\n' \
          +'*******************************************************************************\n'
      se  = str(e)
    print msg
...
if __name__ == '__main__':
  ConfigurationManager(project = 'ExaFMM').configure([])
