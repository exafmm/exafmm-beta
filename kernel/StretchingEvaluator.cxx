  ...
  for( CI=cells.begin(); CI!=cells.end(); ++CI ) {              // Loop over target cells
    if( !lists[CI-CI0].empty() ) {                              //  If the interation list is not empty
      ...
      for( B_iter B=BI0; B!=BIN; ++B ) {                        //   Loop over target bodies
        B->dQdt[0] += targetHost[6*(begin+B-BI0)+0];            //    Copy change rate of x vortex strength from GPU buffer
        B->dQdt[1] += targetHost[6*(begin+B-BI0)+1];            //    Copy change rate of y vortex strength from GPU buffer
        B->dQdt[2] += targetHost[6*(begin+B-BI0)+2];            //    Copy change rate of z vortex strength from GPU buffer
...
void Evaluator::evalP2P(Bodies &ibodies, Bodies &jbodies, bool onCPU) {// Evaluate P2P
  ...
  if( onCPU ) {                                                 // If calculation is to be done on CPU
    ...
    for( B_iter B=BI0; B!=BIN; ++B ) {                          //  Loop over target bodies
      B->dQdt[0] += targetHost[6*(B-BI0)+0];                    //   Copy change rate of x vortex strength from GPU buffer
      B->dQdt[1] += targetHost[6*(B-BI0)+1];                    //   Copy change rate of y vortex strength from GPU buffer
      B->dQdt[2] += targetHost[6*(B-BI0)+2];                    //   Copy change rate of z vortex strength from GPU buffer
    }                                                           //  End loop over target bodies
