...
__device__ void BiotSavartP2M_core(double *target, double rho, double alpha, double beta,
                                   double *sourceShrd, int ithread) {
...
__global__ void BiotSavartP2M_GPU(int *keysGlob, int *rangeGlob, double *targetGlob, double *sourceGlob) {
...
__device__ void BiotSavartM2M_core(double *target, double beta, double *factShrd, double *YnmShrd, double *sourceShrd) {
...
__global__ void BiotSavartM2M_GPU(int *keysGlob, int *rangeGlob, double *targetGlob, double *sourceGlob) {
  int keys = keysGlob[blockIdx.x];
  int numList = rangeGlob[keys];
  double target[6] = {0, 0, 0, 0, 0, 0};
  __shared__ double sourceShrd[6*THREADS];
...
__device__ void BiotSavartM2L_core(double *target, double  beta, double *factShrd, double *YnmShrd, double *sourceShrd) {
  int j = floor(sqrtf(2*threadIdx.x+0.25)-0.5);
  int k = 0;
  for( int i=0; i<=j; ++i ) k += i;
  k = threadIdx.x - k;
...
__global__ void BiotSavartM2L_GPU(int *keysGlob, int *rangeGlob, double *targetGlob, double *sourceGlob) {
  int keys = keysGlob[blockIdx.x];
  int numList = rangeGlob[keys];
...
__device__ void BiotSavartM2P_core(double *target, double r, double theta, double phi, double *factShrd, double *sourceShrd) {
  double x = cosf(theta);
  double y = sinf(theta);
...
__global__ void BiotSavartM2P_GPU(int *keysGlob, int *rangeGlob, double *targetGlob, double *sourceGlob) {
  int keys = keysGlob[blockIdx.x];
  int numList = rangeGlob[keys];
  double D0 = -constDevc[0];
  double targetX[3];
...
__device__ inline void BiotSavartP2P_core(double *target, double *targetX, double *sourceShrd, double3 d, int i) {
  d.x += targetX[0];
  ...
  double cutoff = FOURPI * SQRT_R2_1 * SQRT_R2_1 * SQRT_R2_1 * ( 1.0f - r
...
__global__ void BiotSavartP2P_GPU(int *keysGlob, int *rangeGlob, double *targetGlob, double *sourceGlob) {
  int keys = keysGlob[blockIdx.x];
  int numList = rangeGlob[keys];
  double D0 = -constDevc[0];
  double targetX[3];
...
__device__ void BiotSavartL2L_core(double *target, double beta, double *factShrd, double *YnmShrd, double *sourceShrd) {
  int j = floor(sqrtf(2*threadIdx.x+0.25)-0.5);
  int k = 0;
  for( int i=0; i<=j; ++i ) k += i;
  k = threadIdx.x - k;
...
__global__ void BiotSavartL2L_GPU(int *keysGlob, int *rangeGlob, double *targetGlob, double *sourceGlob) {
  int keys = keysGlob[blockIdx.x];
  int numList = rangeGlob[keys];
  double target[6] = {0, 0, 0, 0, 0, 0};
  __shared__ double sourceShrd[6*THREADS];
...
__device__ void BiotSavartL2P_core(double *target, double r, double theta, double phi, double *factShrd, double *sourceShrd) {
  double x = cosf(theta);
  double y = sinf(theta);
  if( fabs(y) < EPS ) y = 1 / EPS;
  double s = sqrtf(1 - x * x);
...
__global__ void BiotSavartL2P_GPU(int *keysGlob, int *rangeGlob, double *targetGlob, double *sourceGlob) {
  int keys = keysGlob[blockIdx.x];
  int numList = rangeGlob[keys];
  double targetX[3];
  double target[3] = {0, 0, 0};
