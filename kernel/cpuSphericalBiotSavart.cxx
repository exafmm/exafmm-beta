    ...
    for( int k=0; k<=j; ++k ) {
      ...
      for( int n=j; n!=P; ++n ) {
        for( int m=j+k-n; m<0; ++m ) {
          ...
          const int nms  = n * (n + 1) / 2 - m;
          for( int d=0; d!=3; ++d ) {
            L[d] += std::conj(CJ->L[3*nms+d])*Ynm[jnkm]*double(ODDEVEN(k)*Anm[jnkm]*Anm[jk]/Anm[nm]);
          }
        }
        for( int m=0; m<=n; ++m ) {
          if( n-j >= abs(m-k) ) {
            ...
            const int nms  = n * (n + 1) / 2 + m;
            for( int d=0; d!=3; ++d ) {
              L[d] += CJ->L[3*nms+d]*std::pow(I,double(m-k-abs(m-k)))*Ynm[jnkm]*double(Anm[jnkm]*Anm[jk]/Anm[nm]);
            }
          }
        }
      }
