...
#define VDW_SHIFT      2            // vdw shift or switch function 
                                    // MD_LJ_05SIGMA_8SIGMA must be 2
                                    //   0 -- no shift (standard Lennard Jones)
                                    //   1 -- VSHIFT in CHARMM
                                    //   2 -- VSWITCH 
#define VDW_SHIFT_CORRECT           // old VDW_SHIFT function for force was wrong
#define CUTON_DEFAULT  8.5          // default CUTON parameter for VSWITCH
...
#elif 0 // 0.01 - 12A cutoff
#define MD_LJ_R2MIN 0.0001f
#define MD_LJ_R2MAX 144.0f
#elif 0 // 0.01 - 20A cutoff
#define MD_LJ_R2MIN 0.0001f
#define MD_LJ_R2MAX 400.0f
#elif 0 // 0.01 - 100A cutoff
#define MD_LJ_R2MIN 0.0001f
...
/*
  r2rscale_1 = r * r / rscale                          (cutoff)
  r2rscale_3 = r2rscale_1 * r2rscale_1 * r2rscale_1    (dn6)
 */
__inline__ double vg_mul_double(double x, double y);
__inline__ float  vg_mul_float (float  x, float  y);
__inline__ double vg_add_double(double x, double y);
__inline__ float  vg_add_float (float  x, float  y);

#define VDW_FORCE(r2rscale_1,r2rscale_3,one,two,mul,add)			\
          (r2rscale_1)*(mul)((r2rscale_3),(add)((two)*(r2rscale_3),-(one)))
#define VDW_ENERGY(r2rscale_3,one,mul,add)			\
          (mul)((r2rscale_3),(add)((r2rscale_3),-(one)))
#ifdef VDW_SHIFT_CORRECT
#define VDW_FORCE_SHIFT(r2rscale_1,r2rscale_3,r2,rcut21,shift,one,two,three,mul,add,tmp1) \
          (r2rscale_1)*((mul)((r2rscale_3),(add)((two)*(r2rscale_3),-(one)))-(mul)((shift),(add)((three)*(shift),-(two))))
#else
#define VDW_FORCE_SHIFT(r2rscale_1,r2rscale_3,r2,rcut21,shift,one,two,three,mul,add,tmp1) \
          ((tmp1)=(r2),(tmp1)*=(rcut21),(tmp1)*=(tmp1)*(tmp1),			\
           (r2rscale_1)*((mul)((r2rscale_3),(add)((two)*(r2rscale_3),-(one)))-(mul)((tmp1)*(shift),(add)((two)*(shift),-(one)))) \
	  )
#endif
#define VDW_ENERGY_SHIFT(r2rscale_1,r2rscale_3,r2,rcut21,shift,one,two,three,mul,add,tmp1) \
          ((tmp1)=(r2),(tmp1)*=(rcut21),(tmp1)*=(tmp1)*(tmp1),			\
           (add)( (mul)((r2rscale_3),(add)((r2rscale_3),-(one))),	\
                  (tmp1)*(shift)*(add)((two)*(shift),-(one))		\
                  - (shift)*(add)((three)*(shift),-(two)))		\
	  )
#define VDW_FORCE_SWITCH(r2rscale_1,r2rscale_3,r2,rcut21,shift,one,two,three,mul,add,tmp1,tmp2) \
          ((tmp1)=(r2max)-(r2),(tmp1)/=((r2max)-(shift))*((r2max)-(shift))*((r2max)-(shift)),\
	   (tmp2)=(tmp1)*((r2max)-(r2))*((r2max)-(three)*(shift)+(two)*(r2)), \
	   ( (r2rscale_3) * ((r2rscale_3) - (one)) * (three) * (two) * (two) * ((shift) - (r2)) * (tmp1) \
           - (three) * (two) * (r2rscale_3) * ((r2rscale_3) + ((r2rscale_3) - (one)) * (tmp2)) * (tmp2)/(r2) ) \
	  )
#define VDW_ENERGY_SWITCH(r2rscale_1,r2rscale_3,r2,shift,one,two,three,mul,add,tmp1) \
	  ((tmp1)=(r2max)-(r2),(tmp1)=(tmp1)*(tmp1)*((r2max)-(three)*(shift)+(two)*(r2)), \
	   (tmp1)/=((r2max)-(shift))*((r2max)-(shift))*((r2max)-(shift)),\
	   (mul)((r2rscale_3),(add)((r2rscale_3),-(one))) * (tmp1)\
          )
