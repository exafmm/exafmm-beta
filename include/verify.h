...
namespace exafmm {
  ...
  public:
    ...
    bool regression(uint64_t key, bool time, int iteration, double value, double value2=0) {
      ...
      if ((record[key] == 0 || average <= threshold) && (average < 1e-3 || time)) { // If new record or pass
      ...
      if (secondValue) {                                        // If second value
        ...
        if ((record2[key] == 0 || average2 <= threshold) && (average2 < 1e-2 || time)) { // If new record2 or pass
          pass &= true;                                         //  Change flag to pass
