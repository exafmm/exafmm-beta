...
protected:
  ...
  Cells  sticks;                                                // Twigs that collide with parents of other twigs
...
protected:
  ...
  void sortCells(bool ascend, int begin=0, int end=0) {         // Sort cells according to cell index
  ...
  void bodies2twigs(){                                          // Group bodies into twig cells
...
public:
  ...
  void twigs2cells() {                                          // Link twigs bottomup to create all cells in tree
    ...
    while( !twigs.empty() ) {                                   // Keep poppig twigs until the vector is empty
      while( getLevel(twigs.back().I) != level ) {              //  While cell belongs to a higher level
        sortCells(false,begin,end);                             //   Sort cells at this level
        unique(begin,end);                                      //   Get rid of duplicate cells
